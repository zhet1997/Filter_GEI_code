...
% ======================================================================
function [this, err, dpsi] = updateStochasticProcess( this, hp )%hp是由4格组成的元胞数组
    ...
    if this.options.lowRankApproximation
		...
		[C ,rd] = chol(psi);%如果psi对称正定，则rd=0
        %这里输入的psi是个上三角矩阵，但似乎会直接按对称正定来处理。
		P = 1:n;
		rank = size(C,1);
