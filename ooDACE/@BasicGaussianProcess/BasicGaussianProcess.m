...
classdef BasicGaussianProcess
	...
	properties (Access = public)%修改，，，原来是protected
		...
		%% L1 parameters
		alpha = []; %>< Regression coefficients %回归参数 
		gamma = []; %>< 'Correlation part' coefficients
		
        hyperparameters = []; %>< correlation parameters
        rho = 1;
        
        C = []; %>< Choleski Decomposition of extrinsic%外在的 + intrinsic内在的 matrices
        %是cholesky分解后的下三角矩阵
        ...
        %不懂。。。
		Ft = []; %>< decorrelated model matrix %去相关模型矩阵
        ...
        R_reinterp = []; %>< Reinterpolation version of @c R   %重新插值
	...
	methods( Access = public )
		...
		function this = BasicGaussianProcess(varargin)
			...
			elseif isa(varargin{1}, 'BasicGaussianProcess')
				...
				% theta
				if  any(isinf(this.hyperparameters0) | isnan(this.hyperparameters0))
				    error('Invalid initial hyperparameters. (NaN or Infinity)' );
				end
                
				% bounds
				if ~isempty( this.options.hpBounds )
					if length(this.hyperparameters0) ~= size(this.options.hpBounds, 2 )
						error('Initial hyperparameters and bounds should have the same length.');
					end
					
					if any(isinf(this.options.hpBounds) | isnan(this.options.hpBounds) )
					  error('Invalid hyperparameter bounds. (NaN or Infinity)' );
					end
				end
            ...
            this.optimIdx = ones( 1, 4 );
            
            % disable the optimization parameters
            if isinf( this.options.rho0 )
                this.optimIdx(this.RHO) = false; %%%这个参数是什么意思，为啥是这种结构
            end
            ...
            this.optimNrParameters = ones(1,sum(this.optimIdx,2));%这里定义了this.optimNrParameters！！
            this.optimNrParameters(end) = size( this.hyperparameters0, 2 );%也是根据设置的超参数初始值来的。
    ...
    %% PRIVATE
	methods( Access = protected)%ivate )
    ...
    methods( Static )
        ...
        % ======================================================================
        function options = getDefaultOptions()%获得默认的设定
            options = struct( ...
                'generateHyperparameters0', false, ...
                'hpBounds', [], ... % hyperparameter bounds
