%2019-8-29
function [x_l,x_h] = infill2(opt,type)
option.infill = type;
...
%         mm = mm/(opt.Sample.number_h-opt.Sample.initial_num_h+1);
        ...
        x_l = x0;
    case 'Filter_GEI0.05'
        option.cluster_h = 0.05;
        option.cluster_l = 0.05;
        [x,y]=opt.find_GEI(10);
        x0=x;
        x1=[];
        
        opt.Sample.initial_num_h = 21;%2020-4-7临时修改
        mm_ini = mean(opt.Sample.values_h(1:opt.Sample.initial_num_h));%初始加点平均数
        if size(opt.Sample.values_h,1)>opt.Sample.initial_num_h
        mm = sum(opt.Sample.values_h(opt.Sample.initial_num_h+1:end))+mm_ini;
        mm = mm/(opt.Sample.number_h-opt.Sample.initial_num_h+1);
        else
            mm=mm_ini;
...
%           mm = mean(mm);
        ...
        option.cluster_l = 0.1;
        [x,y]=opt.find_GEI(10);
        x0=x;
        x1=[];
        
        opt.Sample.initial_num_h = 21;%2020-6-20临时修改
        mm_ini = mean(opt.Sample.values_h(1:opt.Sample.initial_num_h));%初始加点平均数
        if size(opt.Sample.values_h,1)>opt.Sample.initial_num_h
        mm = sum(opt.Sample.values_h(opt.Sample.initial_num_h+1:end))+mm_ini;
        mm = mm/(opt.Sample.number_h-opt.Sample.initial_num_h+1);
