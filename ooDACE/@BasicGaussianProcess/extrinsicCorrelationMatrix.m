...
%     [psi ,dummy ,dhp] = feval(this.correlationFcn, this.getHyperparameters(), this.dist);
%     
%     if  isempty([psi ,dummy ,dhp])
%         psi =1;
%         dpsi={0};
%     else
        if nargout > 1
            ...
            for i=1:length(dpsi)
                idx = find(dhp(:,i) ~= 0);
                dpsi{i} = sparse([this.distIdxPsi(idx,1); o], [this.distIdxPsi(idx,2); o], [dhp(idx,i); zeros(n,1)]);
            end
        else
        ...
        idx = find(psi > 0);
        psi = sparse([this.distIdxPsi(idx,1); o], [this.distIdxPsi(idx,2); o], [psi(idx); ones(n,1)]);%保证了对角线都是1
        %S = sparse(i,j,v) 根据 i、j 和 v 三元组生成稀疏矩阵 S，
        %以便 S(i(k),j(k)) = v(k)。max(i)×max(j) 输出矩阵为 length(v) 非零元素分配了空间。
        %sparse 将 v 中下标重复（在i 和 j 中）的元素加到一起。
