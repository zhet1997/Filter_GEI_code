...
% ======================================================================
function [out ,dout] = marginalLikelihood( this, dpsi, dsigma2 )%后面两个参数是梯度相关的，正常用不到；
%这是几个关键的函数文件之一
%这里都直接代入计算是因为模型已经在likehoood中更新过了。
    ...
    if this.optimIdx( 1, this.SIGMA2 )%这用于判断这一项是否参与优化。
        ...
        out = 0.5 .* (n.*log(sum(this.sigma2)) + lnDetPsi);%这个是标准的likehood公式%也说明了此sigma即彼sigma
    end
    
    % Derivative
    if nargout > 1%dout是计算梯度的
