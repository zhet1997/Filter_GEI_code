...
classdef Optimizer  %优化器的基类
	...
	methods
		...
		function this = Optimizer(varargin)   
			% get logger
			import java.util.logging.*%这是调用了什么？？
			...
			elseif nargin >= 2
				this.nvars = varargin{1};%参数个数
				this.nobjectives = varargin{2};%目标个数
			...
			this.LB = -ones( 1, this.nvars );%这里是默认的数值
			this.UB =  ones( 1, this.nvars );
			this.initialPopulation = zeros(1, this.nvars );%为何初始种群数是一个零向量
			this.hints = [];
		end % constructor
