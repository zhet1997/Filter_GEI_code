...
function [F, dF] = regressionMatrix(this, points)
    ...
    F = buildVandermondeMatrix( points, this.regressionFcn, cfix( @powerBase, dim ) );
    %建立范德蒙多矩阵%这个不是matlab内置函数，是作者写在tools工具包里的。
    ...
    % their coefficient.
    if nargout > 1%kriging中输出只有一个，下面暂时不看。
        ...
        for i=1:dim
            ...
            dF(i,:) = buildVandermondeMatrix( points, dRegressionFcn, cfix( @powerBase, dim ) );
