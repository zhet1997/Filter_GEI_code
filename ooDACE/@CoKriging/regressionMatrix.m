...
function [F dF] = regressionMatrix(this, points)
    %这里计算的是Co-Kriging的回归项
	% cokriging implementation
    if exist( 'points', 'var' )%predict时，有具体的point
        if nargout > 1
            [F dF] = this.regressionMatrix@BasicGaussianProcess( points );
            dF = [this.rho{1}.*dF dF];
        else
            F = this.regressionMatrix@BasicGaussianProcess( points );
        end
        
        F = [this.rho{1}.*F F];
    else
        %这里就只有两层，根本算不了多层cokriging
