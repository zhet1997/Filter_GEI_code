...
%         inputStd = std(samples);%计算均方误差%计算时各维度是分开的
% 
%         outputAvg = mean(values);
%         outputStd = std(values);
% 
%         samples = (samples - inputAvg(ones(n,1),:)) ./ inputStd(ones(n,1),:);%对样本进行归一化处理
%         values = (values - outputAvg(ones(n,1),:)) ./ outputStd(ones(n,1),:);%对样本值进行归一化处理
% 
