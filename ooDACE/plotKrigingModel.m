...
% ======================================================================
%2019-3-14进行修改
function h = plotKrigingModel(k)

% samples = {k.getSamplesIdx(1);k.getSamplesIdx(2)};%不能区分高低精度
% values = {k.getValuesIdx(1);k.getValuesIdx(2)};

samples = k.getSamples();%不能区分高低精度
values = k.getValues();

inDim = size(k.getSamples,2);

% if ~exist( 'LB', 'var' )
%    LB = min(samples);
%    UB = max(samples);
% end
 LB = zeros(inDim, 1);%直接给定边界
 UB = ones(inDim, 1);
 %%
if inDim == 1%一维图像
...
%===============================================================================================================
    ...
    if iscell( samples )
        plot3( samples{1,1}(:,1), samples{1,1}(:,2), values{1,1}, 'o', 'MarkerEdgeColor','k', 'MarkerFaceColor','k', 'MarkerSize',10 );
        plot3( samples{2,1}(:,1), samples{2,1}(:,2), values{2,1}, 'o', 'MarkerEdgeColor','r', 'MarkerFaceColor','r', 'MarkerSize',10 );
    else
...
%=========================================================================================
    h(2) = figure;
    contourf( x1, x2, reshape( gridy, density, density ),50 );
    hold on;
...
%=========================================================================================
    h(3) = figure;
    contourf( x1, x2, reshape( grids2, density, density ),50 );
    hold on;
...
%==========================================================================================
% for i=1:2500
% [gridy2(i)] = Testmodel( gridx(i,:),'branin' );
%  b(i) = gridy2(i) + Errormodel(gridx(i,:),1);
%                 b(i) = b(i)/2;
% end
% h(4)= figure;
% contourf( x1, x2, reshape( gridy2, density, density ),50 );
%     hold on;
%=====================================================================================    
    
%  h(5)=figure;
%  contourf( x1, x2, reshape( b, density, density ),50 );
%     hold on;

%===================================================================================
% h(6)=figure;
% contourf( x1, x2, reshape(abs(gridy-gridy2'), density, density ),50 );
%     hold on;
