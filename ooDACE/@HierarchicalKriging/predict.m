...
%this是Kriging类，points是预测位置
	...
	%% Regression function	
    if isa(this.GP{1},'Kriging')||isa(this.GP{1},'BasicGaussianProcess')%查看输入是否为Kriging模型
        F = this.GP{1}.predict( points );
    elseif isa(this.GP{1},'function_handle')%如果输入的是一个句柄%要确保句柄能接受多个输入
        F = this.GP{1}( points );
    end
    
    poly = F * this.alpha;%这就是最后的回归项的计算，我寻找的那一项
	
    %% GP part
    % distance matrix
    corr = this.extrinsicCorrelationMatrix(points); % K*'
    if this.optimIdx(1, this.SIGMA2)%如果存在，就说明已经乘进去了，不需要再乘了。
        gp = (this.sigma2 * corr) * this.gamma;%gamma就是和公式后面几个固定项的乘积；
    else
	...
	if nargout > 1
        ...
        if this.options.reinterpolation
            ...
            if strcmp( class(this), 'CoKriging' ) || this.optimIdx(:,this.SIGMA2)
                tmp = (sum(v.^2,1) - sum(corrt.^2,1))';
                sigma2 = repmat(this.sigma2,nx,1) + repmat(tmp, 1, outDim);
            else
                tmp = (1 + sum(v.^2,1) - sum(corrt.^2,1))';
               sigma2 = repmat(this.sigma2,nx,1) .* repmat(tmp, 1, outDim);
               %sigma2 = repmat(this.GP{1}.sigma2,nx,1) .* repmat(tmp, 1, outDim);
            end
        end
	end
end
